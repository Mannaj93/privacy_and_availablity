...
export const START_GET_AGREEMENTS = `START_GET_PATIENT`;
export const END_GET_AGREEMENTS = `END_GET_PATIENT`;

export const START_GET_AGREEMENTS_HISTORY = `START_GET_PATIENT_HISTORY`;
export const END_GET_AGREEMENTS_HISTORY = `END_GET_PATIENT_HISTORY`;

export const START_CREATE_AGREEMENTS = `START_CREATE_PATIENT`;
export const END_CREATE_AGREEMENTS = `END_CREATE_PATIENT`;

export const START_GET_APPROVALS = `START_GET_APPROVALS`;
export const END_GET_APPROVALS = `END_GET_APPROVALS`;

export const START_APPROVE_AGREEMENT = `START_APPROVE_PATIENT`;
export const END_APPROVE_AGREEMENT = `END_APPROVE_PATIENT`;
...
export function getAgreements(data) {
  ...
  return dispatch => {
    ...
    return axios.get(`${routes.getPatient}?${queryParams}`, headers())
      .then(response => {
        console.log(`Contract fetched successfully`)
        console.log(response.data)
        dispatch(createAction(END_GET_PATIENT, response.data.payload));
...
export function getAgreementHistory(id) {
  ...
  return dispatch => {
    dispatch(createAction(START_GET_PATIENTS_HISTORY));
    dispatch(createAction(START_LOADING))
    return axios.get(`${routes.getAgreementHistory}/${id}`, headers())
      .then(response => {
        console.log(`Contract fetched successfully`)
        console.log("============response.data=============",response.data)
        dispatch(createAction(END_GET_PATIENTS_HISTORY, response?.data?.payload));
        console.log("Response from api==========", response.data)
        console.log(response.data.data)
        return response?.data?.payload
      }).catch(err => {
        if (err.response) {
          console.log("action ======================error-------------------------------------==================", JSON.stringify(err.response))
          dispatch(createAction(END_GET_PATIENTS_HISTORY, null, err.response.data))
          throw err.response.data.message.toString()
...
export function approveAgreement(data) {
  return dispatch => {
    dispatch(createAction(START_APPROVE_PATIENT));
    dispatch(createAction(START_LOADING))
    return axios.post(`${routes.approveAgreement}${data.id}`, data.body, headers())
      .then(resp => {
        ...
        dispatch(createAction(END_LOADING))
        dispatch(createAction(END_APPROVE_PATIENT));
      })
  }
}
...
export function createAgreement(data) {
  return dispatch => {
    dispatch(createAction(START_CREATE_PATIENT));
    dispatch(createAction(START_LOADING))
